...
#
# Modified 2020 by Michael Hansen
...
if [ $stage -le 0 ]; then
    ...
    # Kaldi won't regenerate this even if lexicon.txt has changed
...
if [ $stage -le 1 ]; then
    ...
    if [[ -f data/local/dict/lexicon.txt.gz ]]; then
        zcat data/local/dict/lexicon.txt.gz > data/local/dict/lexicon.txt
    fi
...
if [ $stage -le 2 ]; then
    ...
    zcat lm/lm.arpa.gz | utils/find_arpa_oovs.pl data/lang_test/words.txt  > data/local/lm/oovs_lm.txt
...
if [ $stage -le 3 ]; then
    ...
    for datadir in train test; do
        ...
        mkdir -p data/$datadir/wav.scp exp/make_mfcc_chain/$datadir

        for f in wav.scp utt2spk spk2utt; do
            cp data/$datadir/$f exp/make_mfcc_chain/$datadir/
        done
...
EOF
    ...
    if [[ -f utils/mkgraph_lookahead.sh ]]; then
      utils/mkgraph_lookahead.sh --self-loop-scale 1.0 data/lang_test $dir $dir/graph_lookahead
    fi
fi
